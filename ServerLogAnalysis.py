...
uniqueHosts = hosts.reduceByKey(lambda a, b : a + b).collect()
...
dayGroupedHosts = dayToHostPairTuple.groupByKey().map(lambda x : (x[0], list(x[1])))
dayGroupedHosts = dayGroupedHosts.map(lambda (x,y): (x,set(y)))
dayHostCount = dayGroupedHosts.map(lambda (x,y): (x,len(y)))
dailyHosts = dayHostCount.sortBy(lambda x: x[0]).cache()
...
groupedByDay = dayAndHostTuple.groupByKey().map(lambda x : (x[0], list(x[1]))).map(lambda (x,y): (x,len(y)))

sortedByDay = groupedByDay.sortBy(lambda x: x[0])
...
badEndpointsTop20 = badEndpointsSum.sortBy(lambda x: x[1],ascending=False).collect()[0:20]
...
errHostsTop25 = errHostsSum.sortBy(lambda x: x[1],ascending=True).collect()[0:25]
...
# TODO: Replace <FILL IN> with appropriate code
#print access_logs.first()[2].date().day
...
errDateSorted = errDateSum.sortBy(lambda x: x[0],ascending=True).cache()
...
topErrDate = errDateSorted.sortBy(lambda x: x[1],ascending=False).collect()[0:5]
...
hourRecordsSorted = hourRecordsSum.sortBy(lambda x: x[0],ascending=True).cache()
