...
def parsePoint(line):
    ...
    for num in line.split(','):
        labeledLine.append(float(num))
    return LabeledPoint(labeledLine[0],labeledLine[1:])
...
#print firstPointFeatures[0]
#print np.allclose(expectedX0[0], firstPointFeatures[0],0.0004,0.0004)
...
#print parsedDataInit.first().label-minYear
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData,iterations=numIters,step=alpha,regParam=reg,regType='l2',intercept=True)
...
# TODO: Replace <FILL IN> with appropriate code
#labelsAndPreds = parsedTrainData.map(lambda x: (x[0].label,firstModel.predict(x[0].features)))
...
def twoWayInteractions(lp):
    ...
    """
    currentFeatures = list(lp.features)
    newFeatures = list(itertools.product(lp.features,lp.features))
    for tup in newFeatures:
        tup = tup[0]*tup[1]
        currentFeatures.append(tup)
    return LabeledPoint(lp.label,currentFeatures)
