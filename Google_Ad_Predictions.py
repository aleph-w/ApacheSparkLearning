...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    numRatings = float(len(ratings))
    avgRating = sum(ratings)/numRatings
    return (movie, (numRatings, avgRating))
    #print movie, ratings
...
# TODO: Replace <FILL IN> with appropriate code
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, 8, seed=seed, iterations=iterations, lambda_=regularizationParameter)
...
#print myRatingsRDD.take(3)
#print myUnratedMoviesRDD.first()
...
predictedWithCountsRDD  = (predictedRDD
                           .join(movieCountsRDD))
#print predictedWithCountsRDD.first()
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda (w,((x,y),z)): (x,z,y))
                       .filter(lambda x: x[2] > 75)
                      )
#print ratingsWithNamesRDD.first()
